	...
	return {
		...
		shouldEnableForCurrentPage: function (prefCat, prefName, sketch) {
			...
			var pageNum = parseInt (sketch.metadata.id, 10),	// Use numeric pageNum to check against array
					...
					return pages === true || Array.isArray (pages) && (pages[0] === "all" || pages.includes(pageNum));
			},
...
// Private functions here, utility functions first
	...
	function Widget (name, changeEvent) { // All widgets have a button that must be appropriately shown (when enabled) and highlighted (when active)
		this.name = name;
	...
	function showWidget (doShow) {
		if (arguments.length && !doShow) {	// If there's a false value passed, hide rather than show.
			hideWidget();
		} else {
	...
	function targetControllerToDoc (newNode, forceReposition) {	// Position the widgets in a new sketch node.
		...
		var sketchNode = getSketchNode (newNode),
				...
				buttonNode = $(newNode.parentNode).find(".widget_button"),
	...
	function resizeSketchFrame (sketchDoc) {	// Resize the frame to fit a new sketch
		...
		if ($container.hasClass ("sketch_container")) {
			...
			if (width) {
				width += parseInt ($container.css("border-left-width"),10) + 
...
*/
	...
	function setDomColor (color, gobj) {
		...
		var domNode = $targetNode.find ("[wsp-id='" + gobj.id +"']").not(".wsp-sr-only");	// The domNode exists for text objects and buttons
	...
	styleWidget.postProcessGobj = function (gobj) { // return true if style is changed
		var ret = false,
				cancel = styleWidget.cancelOnExit;
		if (gobj.oldStyle) {  // exists if this gobj's style was changed
			if (cancel) {
...
/* ***** VISIBILITY WIDGET ***** */
	...
	visibilityWidget.postProcessGobj = function (gobj) {
		...
		retVal = gobj.style.hidden && !gobj.wasHidden || !gobj.style.hidden && gobj.wasHidden;
...
/* ***** LABEL WIDGET ***** */
	...
	labelWidget.finalizeLabel = function () {
		...
		function sendEvent(gobj) {  // Was gobj changed? If so, emit an event.
			var currentText = (gobj.genus === "Caption") ? gobj.textMFS : gobj.label,
					changed = currentText !== self.oldLabel ||
	...
	labelWidget.handleTap = function (event, context) {	// cache label info to enable restoring it if user cancels
		...
		if (!newTarget.canEditLabel() || newTarget.genus === "Caption" && !singleLineCaption (newTarget.textMFS)) {
			return;
		}
		showLabelPane ();
 ...
 */
	...
	function noMotionActive () {
		...
		for (ix = 0; ix < list.length; ix++) {
			...
			if (member && member.state === "active") {  // Check whether this member of the motionSet is active
				return false;
			}
		}
		return true;
	...
	function setTraceRenderState (state) {  // Sets the render state for every traced object
	...
	function startGlowing () {  // Makes all traced objects glow PROVIDED the glowBox is checked
		// AND EITHER there's no active motion OR tracing is disabled
		if (traceWidget.glowBox.checked && (noMotionActive() || !$("#wTraceEnabled")[0].checked)) {
	...
	function stopGlowing (arg) {  // Stops all traced objects from glowing, unless tracing is disabled
	...
	traceWidget.setGlowing = function () {	// Set the widget's behavior to match the Glow checkbox.
...
var PAGENUM = (function() {	// define the PAGENUM namespace, for the code that creates and handles page number controls
	...
	function injectButtonElements (sketchDoc) {	// Find any uninitialized .page_buttons element with html matching this doc id, and initialize the buttons.
		...
		if ($btnArea.length === 1) {
			...
			if (logo && getCtl (sketchNode, ".wsp_logo").length === 0) {	// Insert logo only if it's not there already
	...
	return {	// public functions and variables
		...
		resetPage: function (btn) {	// button handler for the Reset Sketch button
