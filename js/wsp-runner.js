...
(function( $ ){
  ...
  function loadFromUrl (sketchUrl, $target, options) {
    if (!sketchUrl)
      sketchUrl = $target.data('url');
    ...
    $.ajax({
        ...
        success: function (text, statusMsg) {
            if (text) {
              ...
              if ($target.is(":visible")) { // If sketch_canvas is still visible, load it.
                loadSketch(text, sketchUrl, $target, options);
              }
              else  { // if sketch_canvas has been hidden, delay it.
                GSP.log ("Load delayed: visibility changed for " + sketchUrl + ".");
                delayLoadFromUrl (sketchUrl, $target, options);
              }
            }
            else {
  ...
  function delayLoadFromUrl (sketchUrl, $target, options) {
    GSP.log("Delaying load of hidden sketch: " + sketchUrl);
/* jshint ignore:start */  
     ...
     fetchFromAttr.observer = new IntersectionObserver (function (entries) {
       entries.forEach(function (entry) {
         var $target = $(entry.target),
             key = $target.data ('url');
         if (entry.isIntersecting && $target.is(":visible")) {
           loadFromUrl ("", $target, fetchFromAttr.optionsList[key]);
    ...
    fetchFromAttr.optionsList[sketchUrl] = options;
/* jshint ignore:end */  
  }
	
	var fetchFromAttr = {
    'observer': null,     // The observer tells us when it's safe to load an originally-hidden sketch.
    'optionsList': {},    // store the options of each delayed-load sketch
	/*
	 ...
	 */
	 'url': function(sketchUrl, $target, options) {
      ...
      if ($target.is(":visible") || !useIntObserver) {
        loadFromUrl (sketchUrl, $target, options);
